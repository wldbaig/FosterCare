...
FC.SiteScript = {
    ...
    ShowNotificationMessage: function () {
        ...
        if (messageJsonString && FC.UtilityFunctions.IsJsonString(messageJsonString)) {
    ...
    AjaxErrorHandler: function () {
        ...
        $(document).ajaxError(function (event, request, options) {
            ...
            if (request.status && request.status === 403) {
                window.location.replace(FC.AppConstants.URL.Action.AccessDenied);
            }
        });
    },
    ...
    ShowMessage: function (objMessage) {
        ...
        switch (objMessage.Type) {
            ...
            case FC.AppConstants.MessageType.Success:
                className = 'bg-success';
                heading = 'Success';
                break;
            case FC.AppConstants.MessageType.Info:
                className = 'bg-info';
                heading = 'Info';
                break;
            case FC.AppConstants.MessageType.Error:
                className = 'bg-danger';
                heading = 'Error';
                break;
            case FC.AppConstants.MessageType.Warning:
    ...
    MessageBox: {
        ...
        ShowSuccess: function (message) {
            FC.SiteScript.ShowMessage({ Message: message, Type: 'Success' });
        },
        ShowError: function (message) {
            FC.SiteScript.ShowMessage({ Message: message, Type: 'Error' });
        },
        ShowWarning: function (message) {
            FC.SiteScript.ShowMessage({ Message: message, Type: 'Warning' });
        },
        ShowInfo: function (message) {
            FC.SiteScript.ShowMessage({ Message: message, Type: 'Info' });
        }
    },
    ...
    AjaxFormEventHandler: {
        onAjaxError: function (xhr, status, entityName) {
            if (xhr.statusText == FC.AppConstants.HttpStatusCode.Forbidden) {
    ...
    KendoEventHandler: {
        ...
        onContentLoad: function (e) {
            ...
            FC.SiteScript.AddRemoveCssClasses();

            //apply tooltip on each element having title attribute
            FC.SiteScript.ApplyKendoTooltip();

            //show/hide section on loading dynamic contents
            FC.SiteScript.ShowHideWebsiteSections();

            //Disable autocomplete option in all textboxes
            FC.SiteScript.SwitchAutoCompleteOnTextBoxes(true);
        },
        ...
        onGridError: function (e) {
            //redirect to access denied page if request is forbidden
            if (e.errorThrown && e.errorThrown.toString().toLowerCase() === FC.AppConstants.HttpStatusCode.Forbidden.toLowerCase() || (e.xhr && e.xhr.status && e.xhr.status == 403)) {

                //Redirecting to access denied page
                window.location.replace(FC.AppConstants.URL.Action.AccessDenied);
            }
            if (e.errorThrown && e.errorThrown.toString().toLowerCase() === FC.AppConstants.HttpStatusCode.InternalServerError.toLowerCase() || (e.xhr && e.xhr.status && e.xhr.status == 500)) {
                FC.SiteScript.MessageBox.ShowError(e.errorThrown);
            ...
            if (e.errors && e.errors && e.errors["ERROR"] && e.errors["ERROR"].errors[0]) {
                ...
                FC.SiteScript.MessageBox.ShowError('An error has occured in this operation');
                //FC.SiteScript.MessageBox.ShowError(FC.SiteScript.GetApplicationMessage(FC.AppConstants.ApplicationMessageKey.Operation_Error));
            }
        },
    ...
    KendoGridCommands: {
        ...
        onEdit: function (e) {
            debugger;
            $(e.container).find("td:last").html("<a href='javascript: void(0)'   onclick='FC.SiteScript.KendoGridCommands.UpdateRow(this)' title='update'><span class='icon-grid icon-update'></a> " +
                "<a href='javascript: void(0)'   onclick='FC.SiteScript.KendoGridCommands.CancelRow(this)' title='cancel'><span class='icon-grid icon-cancel'></a>");
        },
        onSave: function (e) {
            debugger;
    ...
    ShowHideWebsiteSections: function () {
        ...
        if (!FC.AppVar.AppSetting.IsNavigationMenuPinned) {
    ...
    GetApplicationMessage: function (messageKey) {
        ...
        var appMessages = localStorage.getItem(FC.AppConstants.LocalStorageKey.AppMessages);

        if (appMessages == null) {
            var actionUrl = FC.AppConstants.URL.Action.Common.GetApplicationMessages;
            ServiceManager.Get(actionUrl, false, GetApplicationMessagesCallBack, null, false, false, null);
            debugger;
            appMessages = localStorage.getItem(FC.AppConstants.LocalStorageKey.AppMessages)
        }
...
function GetApplicationMessagesCallBack(response, param) {
    ...
    if (response && response.length > 0 && response[0]) {
        ...
        localStorage.setItem(FC.AppConstants.LocalStorageKey.AppMessages, appMessages);
    }
}
