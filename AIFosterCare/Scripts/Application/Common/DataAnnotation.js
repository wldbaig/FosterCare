    ...
    showErrors: function (errorMap, errorList) {
        ...
        for (var i = 0; i < errorList.length; i++) {
            ...
            if ($(id).data("kendoMultiSelect") != undefined
                ...
                FC.UtilityFunctions.ApplyDangerTooltip($(id).parent(), error.message)
                $(id).parent().addClass('kendo-validation-error')

            } else if ($(id).data("kendoEditor") != undefined) {
                var editor = $(id).closest('table');
                FC.UtilityFunctions.ApplyDangerTooltip(editor, error.message)
                editor.addClass('kendo-validation-error')
            } else {
                FC.UtilityFunctions.ApplyDangerTooltip($(id), error.message)
            }
            ...
            if (isFirstControll) {
                ...
                if (clickEvent == undefined) {
                    ...
                    $(submitButton).on('click', function (e) {
                        FC.UtilityFunctions.FocusOnErrorControls($(this).closest('form'));
                    });//.trigger('click');
