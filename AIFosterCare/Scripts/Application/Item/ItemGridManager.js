...
function ItemGridManager() {
    ...
    function CreatePopupWindow() {
        ...
        domElement.kendoWindowElement.kendoWindow({
            ...
            error: function (e) {
                if (e.xhr.statusText == FC.AppConstants.HttpStatusCode.Forbidden) {
    ...
    function onPopupWindow_Refresh() {
        InitializeVariables();
        FC.UtilityFunctions.RebindUnobtrusiveValidation();
        FC.UtilityFunctions.IgnoreDefaultValidationRule(domElement.formProgramSubject);
    }
    ...
    this.onChangeItemStatus = function (source, id) {
        debugger;
        FC.SiteScript.CustomConfirmationBox("Are you sure, you want to change status?", onChangeItemStatusOkCallback, null, { source: source, id: id });
    ...
    function onChangeItemStatusOkCallback(objInfo) {
        debugger;
        if (objInfo != null) {
            var actionUrl = FC.AppConstants.URL.Action.Item.ChangeItemStatus + "?id=" + objInfo.id;
    ...
    //    debugger;
    //    FC.SiteScript.CustomConfirmationBox("Are you sure, you want to add this to order?", onAddItemToOrderOkCallback, null, { source: source, id: id });
    //    //"Are you sure, you want to change program status?"
    //};
    ...
    this.OpenPopupWindow = function (itemId) {
        ...
        globalVar.popupWindow.refresh({
            url: FC.AppConstants.URL.Action.Order.AddToOrderPartial,
            data: { itemId: itemId  }
        });
    ...
    this.SaveOrder = function (source) {
        ...
        var formActionURL = FC.AppConstants.URL.Action.Order.AddItemToOrder;
    ...
    function AddItemToOrderCallBack(response, param) {
        ...
        if (response && response.length > 0 && response[0]) {
            ...
            if (messageModel.Type == FC.AppConstants.MessageType.Success) {
               
                that.ClosePopupWindow();
            }

            FC.SiteScript.MessageBox.ShowSuccess(messageModel.Message);
        }
    }
    ...
    //    debugger;
    //    if (objInfo != null) {
    //        var actionUrl = FC.AppConstants.URL.Action.Order.AddItemToOrder + "?id=" + objInfo.id;
