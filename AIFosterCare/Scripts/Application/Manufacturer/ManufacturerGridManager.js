...
function ManufacturerGridManager() {
    ...
    this.onChangeManufacturerStatus = function (source, id) {
        debugger;
        FC.SiteScript.CustomConfirmationBox( "Are you sure, you want to change status?" , onChangeManufacturerStatusOkCallback, null, { source: source, id: id });
    ...
    function onChangeManufacturerStatusOkCallback(objInfo) {
        debugger;
        if (objInfo != null) {
            var actionUrl = FC.AppConstants.URL.Action.Manufacturer.ChangeManufacturerStatus + "?id=" + objInfo.id;
