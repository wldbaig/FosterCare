...
function DistributorGridManager() {
    ...
    this.onChangeDistributorStatus = function (source, id) {
        debugger;
        FC.SiteScript.CustomConfirmationBox("Are you sure, you want to change status?", onChangeDistributorStatusOkCallback, null, { source: source, id: id });
    ...
    function onChangeDistributorStatusOkCallback(objInfo) {
        debugger;
        if (objInfo != null) {
            var actionUrl = FC.AppConstants.URL.Action.Distributor.ChangeDistributorStatus + "?id=" + objInfo.id;
